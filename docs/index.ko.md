...
-   :material-television-guide:{ .lg .middle } __안내식 첫 백업__
    ...
    채널 URL, `@handle`, `UC…` ID를 붙여넣고 미리 본 뒤 등록하고, 남은 영상을
    자동으로 보관하는 자동 다운로드 스케줄을 시작합니다.
...
-   :material-shield-check:{ .lg .middle } __기본은 안전__
    ...
    실제 다운로드는 자동 다운로드 스케줄을 시작(또는 고급 수동 테스트를 실행)하기
    전까지 꺼져 있습니다. 파일시스템은 결코 파괴적으로 다시 쓰이지 않습니다.
