...
| DSM 호환 x86 NAS | 메인테이너 스모크 테스트 | 영구 메타데이터/미디어/런타임 마운트, 채널 등록, 자동 백업, 재시작, 라이브러리 색인 |
| `linux/amd64` 이미지 | 배포 및 pull 검증 | 멀티 아키텍처 릴리스 manifest와 Docker Hub/GHCR 익명 pull 스모크 |
| `linux/arm64` 이미지 | 배포 및 pull 검증 | 멀티 아키텍처 릴리스 manifest와 Docker Hub/GHCR 익명 pull 스모크 |
...
- Compose v2를 지원하는 Docker Engine
