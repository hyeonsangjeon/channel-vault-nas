...
// the registration flow reads consistently. Mirrors e2e/channel-registration-flow.spec.ts.
...
async function analyzeChannel(page: Page) {
  const addChannel = page.getByRole("button", { name: "Add your channel" }).first();
  if (await addChannel.isVisible().catch(() => false)) await addChannel.click();
  const input = page.locator(".registration-command input").first();
  await input.waitFor({ timeout: 15000 });
  await input.click();
  await input.fill("");
  await input.pressSequentially("https://www.youtube.com/@wingnut0310", { delay: 42 });
  await page.locator('.registration-command button[type="submit"]').first().click();
  await page.locator(".probe-preview").first().waitFor({ timeout: 15000 });
}
...
test("record tutorial segments", async ({ browser }) => {
  ...
  // ---- 03-firstrun (empty state -> channel registration preview) ----------
  {
    const { context, page } = await newRec(browser, "en");
    const t = Date.now();
    await setupWizardMock(page);
    await goto(page, "#/dashboard");
    await page.getByText("See the full NAS console in 10 seconds").first().waitFor({ timeout: 15000 });
    await page.waitForTimeout(600);
    await page.evaluate(() => {
      const add = Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.includes("Add your channel"));
      if (add) (add as HTMLElement).id = "cvn-anchor-add-channel";
    });
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      c.__cvn.setBadge(2);
      await c.__cvn.moveToSel("#cvn-anchor-add-channel", 900);
      c.__cvn.ringSel("#cvn-anchor-add-channel", 6, 12);
      await c.__cvn.sleep(400);
      c.__cvn.ripple();
    });
    await page.locator("#cvn-anchor-add-channel").click();
    await page.locator(".registration-panel").first().waitFor({ timeout: 15000 });
    await page.evaluate(() => {
      const el = document.querySelector(".registration-command input");
      if (el) (el as HTMLElement).id = "cvn-anchor-src";
      const btn = document.querySelector('.registration-command button[type="submit"]');
      if (btn) (btn as HTMLElement).id = "cvn-anchor-preview";
    });
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
    ...
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      await c.__cvn.moveToSel("#cvn-anchor-preview", 850);
      c.__cvn.ringSel("#cvn-anchor-preview", 6, 12);
      await c.__cvn.sleep(500);
      c.__cvn.ripple();
    });
    await page.locator("#cvn-anchor-preview").click();
    await page.locator(".probe-preview").first().waitFor({ timeout: 15000 });
    await page.waitForTimeout(700);
    // point back at the safe demo CTA to show it remains optional
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      const demo = Array.from(document.querySelectorAll("button")).find((button) => button.textContent?.includes("Load safe demo")) as HTMLElement | undefined;
      if (demo) {
        demo.id = "cvn-anchor-demo";
        demo.scrollIntoView({ block: "center", behavior: "smooth" });
        await c.__cvn.sleep(500);
        await c.__cvn.moveToSel("#cvn-anchor-demo", 900);
        c.__cvn.ringSel("#cvn-anchor-demo", 6, 12);
      }
    });
  ...
  // ---- 03b-overview (channel preview review) -----------------------------
  {
    const { context, page } = await newRec(browser, "en");
    const t = Date.now();
    await setupWizardMock(page);
    ...
    await page.evaluate(async () => {
      ...
      const plan = document.querySelector(".probe-preview") as HTMLElement | null;
      if (plan) plan.scrollIntoView({ block: "center", behavior: "smooth" });
      await c.__cvn.sleep(700);
      // pan across the preview stats
      const metrics = Array.from(document.querySelectorAll(".probe-stats span")).slice(0, 4);
    ...
    // rest on the preview identity, then the preview list
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      const identity = document.querySelector(".probe-identity") as HTMLElement | null;
      if (identity) {
        identity.id = "cvn-anchor-identity";
        await c.__cvn.moveToSel("#cvn-anchor-identity", 800);
        c.__cvn.ringSel("#cvn-anchor-identity", 8, 14);
      }
    });
    await page.waitForTimeout(1600);
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      const list = document.querySelector(".probe-videos") as HTMLElement | null;
      if (list) {
  ...
  // ---- 04-register (Register channel -> schedule panel) ------------------
  {
    const { context, page } = await newRec(browser, "en");
    const t = Date.now();
    await setupWizardMock(page);
    ...
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      c.__cvn.setBadge(3);
      const start = document.querySelector(".register-action") as HTMLElement | null;
      if (start) {
        start.id = "cvn-anchor-register";
        start.scrollIntoView({ block: "center", behavior: "smooth" });
        await c.__cvn.sleep(600);
        await c.__cvn.moveToSel("#cvn-anchor-register", 950);
        c.__cvn.ringSel("#cvn-anchor-register", 6, 12);
        await c.__cvn.sleep(600);
        c.__cvn.ripple();
      }
    });
    await page.locator(".register-action").first().click();
    await page.locator(".channel-automation-panel").first().waitFor({ timeout: 15000 });
    await page.waitForTimeout(700);
    await page.evaluate(() => (window as unknown as { __cvn: any }).__cvn.clearRing());
    // point at the schedule counts (total / downloaded / remaining)
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      const grid = document.querySelector(".channel-automation-count-grid") as HTMLElement | null;
      if (grid) {
    ...
    // highlight the schedule registration button (do NOT click — no real download here)
    await page.evaluate(async () => {
      const c = window as unknown as { __cvn: any };
      const btn = document.querySelector(".channel-automation-panel .primary-action") as HTMLElement | null;
      if (btn) {
        btn.id = "cvn-anchor-schedule";
        await c.__cvn.moveToSel("#cvn-anchor-schedule", 900);
        c.__cvn.ringSel("#cvn-anchor-schedule", 6, 12);
        await c.__cvn.sleep(600);
        c.__cvn.ripple();
      }
    });
