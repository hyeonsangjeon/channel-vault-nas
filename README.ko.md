...
- **`archive.txt`의 판단을 보여줍니다.** 다운로드됨, 없음, 대기, 건너뜀 상태가
  명령행 옵션 안에 숨지 않습니다.
- **사라진 업로드를 포착합니다.** 이미 보관한 영상이 YouTube에서 사라지면
  (삭제·비공개·차단) 다음 동기화에서 감지해 이제 NAS가 마지막 사본을 가졌음을
  확인합니다. 오직 내 디스크에만 남은 영상 목록을 보존 매니페스트로 내보낼 수
  있습니다.
...
| **기존 미디어를 재사용하고 `archive.txt` 판단을 보여주며, 사라진 영상의 마지막 사본을 NAS가 가졌는지 알려주는 디스크 우선 채널 아카이빙** | **Channel Vault NAS** |
...
**0.3.0으로 알파를 벗어나 활발히 개발 중입니다.** 기본 화면은 홈·채널·저장한
