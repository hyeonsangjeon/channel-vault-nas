...
- **`archive.txt`의 판단을 보여줍니다.** 다운로드됨, 없음, 대기, 건너뜀 상태가
  명령행 옵션 안에 숨지 않습니다.
- **원본 목록의 변화를 확인합니다.** 저장한 영상이 원본의 공개 목록에서
  확인되지 않으면 로컬 사본을 보존된 영상으로 표시하고 목록을 내보낼 수 있습니다.
  삭제 여부나 다른 사본의 부재를 증명하지는 않습니다. 기본 조회 범위는 최근
  업로드 최대 500개이며, 시간 기준은 마지막 확인 이후 24시간입니다.
...
| **기존 미디어를 재사용하고 `archive.txt` 판단을 보여주며, 원본 목록에서 확인되지 않는 저장 영상을 알려주는 디스크 우선 채널 아카이빙** | **Channel Vault NAS** |
...
**알파를 벗어나 활발히 개발 중이며, 현재 릴리스는 0.3.1입니다.** 기본 화면은 홈·채널·저장한
